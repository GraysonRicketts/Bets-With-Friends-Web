...
function calculateOverallUserScore(userId: uuid, bets: Bet[]) {
    ...
    const pooledWinners: PooledWinner[] = wonBets.map(b => {
        const winners = b.wagers.filter(w => w.option.id === b.outcome?.id).map(w => w.user);
        const poolTotal = b.wagers.map(w => w.amount).reduce((pv, cv) => pv + cv, 0);
        return {
            winners,
            poolTotal
        }
...
export const ScoreScreen: React.FC<Props> = ({ group }) => {
    const overallScores: { user: User, overallScore: points }[] = group.members.map(user => {
    ...
    return <>
        ...
        {overallScores.map(os => <Typography>{os.user.name} - {os.overallScore}</Typography>)}
    </>
}