import { Box, Button, Modal, TextField, IconButton, Typography, Select, MenuItem, FormControl, InputLabel } from "@mui/material";
    ...
    return <Modal open={isOpen} onClose={() => {
        ...
        <Box sx={modalStyle}>
            <div>
                ...
                {categories.length && !showAddCategory && <FormControl variant="standard">
                    ...
                    >
                        {categories.map(c => <MenuItem value={c.name} key={`category_id_${c.id}`}>{c.name}</MenuItem>)}
                    </Select></FormControl>}
                {!showAddCategory &&
